            ...
            <section class="content">
                <div class="container-fluid">
                    ...
                    $stmt2 = $conn->query("SELECT * FROM db_posttest INNER JOIN  db_posttest_choice ON  db_posttest.post_id = db_posttest_choice.post_id 
                                                                     WHERE db_posttest.main_id = '$dataclass[main_id]'");
                    $stmt2->execute();
                    ...
                    ?>
                                <form action="check_answer_posttest" method="post">
                                    <div class="card">
                                        <h5 class="card-header">แบบทดสอบหลังเรียน <?=$dataclass['name'];?></h5>
                                        <input type="hidden" name="user_id" id="" value="<?= $data["user_id"]; ?>">
                                        <input type="hidden" name="s_main_id" id="" value="<?= $dataclass["main_id"]; ?>">


                                        <?php

                                        foreach ($allpretest as $dataposttest) {
                                            $choicenumber++;
                                        ?>
                                            <div class="card-body">
                                                <input type="hidden" name="a_post_id[]" id="" value="<?= $dataposttest["post_id"]; ?>">
                                                <div class="box-title">
                                                    <p>ข้อ<?= $choicenumber; ?>
                                                        <?= $dataposttest["question"]; ?></p>
                                                    <div class="item">
                                                        <ul>
                                                            <li>ก.<?= $dataposttest['choice1']; ?></li>
                                                            <li>ข.<?= $dataposttest['choice2']; ?></li>
                                                            <li>ค.<?= $dataposttest['choice3']; ?></li>
                                                            <li>ง.<?= $dataposttest['choice4']; ?></li>
                                                        </ul>
                                                    </div>
